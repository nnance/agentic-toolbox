...
A minimalist TypeScript SDK that provides a unified abstraction layer for text generation across multiple LLM providers (Ollama, Anthropic, OpenAI) with built-in support for tool calling and chat completions.
...
const result = await executeToolLoop(llm, {
  model: "gpt-4",
  messages: [{ role: "user", content: "Calculate 25 * 4, then add 15" }],
