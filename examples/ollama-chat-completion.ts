...
async function demonstrateChatCompletion() {
  ...
  try {
    const response1 = await ollama.generateChatCompletion({
      ...
      messages: [
        ...
        { role: 'user', content: 'Explain what prime numbers are in simple terms.' }
      ]
    });
  ...
  try {
    ...
    if (response3a.toolCalls && response3a.toolCalls.length > 0) {
      const messages: Message[] = [
        { role: 'user', content: prompt },
        { role: 'assistant', content: response3a.text, toolCalls: response3a.toolCalls }
      ];
      ...
      for (const toolCall of response3a.toolCalls) {
        ...
        try {
          const result = calculate(toolCall.arguments as { operation: string; numbers: number[] });
  ...
  console.log('   (e.g., qwen3:30b, llama3.1). You can install it with: ollama pull qwen3:30b');
}
